...
In addition to the `shovel.env` above, you may need a CA Certfile to connect to your rabbit instance.
...
* Running locally using the Docker config
  * The docker-compose configs are located in `Docker/docker-compose-configs` directory.
...
* Running natively via NodeJS
  * **.env** (same template as **shovel.env**) in **/app**
  * Alternately, you can pass in a specific env file **shovel.ENVNAME.env** by adding an arg to the function invocation as demo'ed below
  * possibly **ca_certificate.pem** (if needed) in **/app/conf**
  * Be sure you've done `npm i` and `nvm i`
  * `node index.js` or `node index.js ./conf/shove.331.batch.env`
...
* tip: if you happen to be trying to receive messages from a locally running rabbit instance (such as a message-broker service from ipb-dev-standup docker-compose) you're going to need to adjust your .env to be pointing to the localhost (obviously) but also to choose the insecure amqpProtocol and associated port, as per those env settings described above)
  * amqpHost=localhost
  * amqpPort=5672
  * amqpProtocol=amqp
